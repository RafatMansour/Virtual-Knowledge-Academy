...
async function getEnrollments(): Promise<{ enrollments: Enrollment[] }> {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'
  const res = await fetch(`${baseUrl}/api/enrollments`, { cache: 'no-store' })
  if (!res.ok) return { enrollments: [] }
  return res.json()
}

type Progress = { id: string; user_id: string; lesson_id: string; is_completed: boolean }

async function getProgress(): Promise<{ progress: Progress[] }> {
  const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'
...
export default async function DashboardPage() {
  const [{ enrollments }, { progress }] = await Promise.all([getEnrollments(), getProgress()])
